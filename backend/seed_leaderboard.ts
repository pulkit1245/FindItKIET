...
async function seed() {
    ...
    try {
        ...
        if (error.status === 401 || error.status === 404) {
            ...
            try {
                ...
                console.error('❌ Registration failed:', regError.data || regError);
                return;
            }
        } else {
    ...
    for (let i = 0; i < ITEMS_TO_CREATE; i++) {
        ...
        try {
            ...
            console.log(`✅ Created item ${i + 1}/${ITEMS_TO_CREATE}: ${item.title}`);
        } catch (error: any) {
            console.error(`❌ Failed to create item ${i + 1}:`, error.data || error);
        }
    ...
    console.log('🎉 Seeding complete! User should be on top of leaderboard.');
}
